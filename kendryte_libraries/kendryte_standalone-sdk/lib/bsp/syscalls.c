...
void sys_register_putchar(sys_putchar_t putchar) { sys_putchar = putchar; }

void sys_register_getchar(sys_getchar_t getchar) { sys_getchar = getchar; }

void sys_stdin_flush(void) {
  if (sys_getchar)
    while (sys_getchar() != EOF) continue;
}
...
static int sys_nosys(long a0, long a1, long a2, long a3, long a4, long a5,
                     unsigned long n) {
  UNUSED(a3);
  UNUSED(a4);
  UNUSED(a5);
...
static int sys_success(void) { return 0; }
...
static ssize_t sys_write(int file, const void *ptr, size_t len) {
  ...
  if (STDOUT_FILENO == file || STDERR_FILENO == file) {
    ...
    while (length-- > 0 && data != NULL) {
      if (sys_putchar) sys_putchar(*(data++));
    }
...
static int sys_fstat(int file, struct stat *st) {
  ...
  if (st != NULL) memset(st, 0, sizeof(struct stat));
  /* Return the result */
  res = -ENOSYS;
  /**
...
static int sys_gettimeofday(struct timeval *tp, void *tzp) {
  ...
  if (tp != NULL) {
    uint64_t clint_usec =
        clint->mtime /
        (sysctl_clock_get_freq(SYSCTL_CLOCK_CPU) / CLINT_CLOCK_DIV / 1000000UL);
...
handle_ecall(uintptr_t cause, uintptr_t epc, uintptr_t regs[32],
  ...
  static uintptr_t (*const syscall_table[])(
      long a0, long a1, long a2, long a3, long a4, long a5, unsigned long n) = {
...
handle_misaligned_load(uintptr_t cause, uintptr_t epc, uintptr_t regs[32],
  ...
  if (offset >> 11) offset = -((offset & 0x3FF) + 1);
  ...
  LOGV(TAG,
       "misaligned load recovered at %08lx. "
       "len:%02d,addr:%08lx,reg:%02d,data:%016lx,signed:%1d,float:%1d",
       (uint64_t)epc, len, (uint64_t)addr, dst, data_load, !unsigned_, fpu);
...
handle_misaligned_store(uintptr_t cause, uintptr_t epc, uintptr_t regs[32],
  ...
  LOGV(TAG,
       "misaligned store recovered at %08lx. "
       "len:%02d,addr:%08lx,reg:%02d,data:%016lx,float:%1d",
       (uint64_t)epc, len, (uint64_t)addr, src, data_store, fpu);
...
uintptr_t handle_syscall(uintptr_t cause, uintptr_t epc, uintptr_t regs[32],
                         uintptr_t fregs[32]) {
  static uintptr_t (*const cause_table[])(uintptr_t cause, uintptr_t epc,
                                          uintptr_t regs[32],
                                          uintptr_t fregs[32]) = {
...
size_t get_free_heap_size(void) { return (size_t)(&_heap_end[0] - _heap_cur); }
