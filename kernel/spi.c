                           ...
                           uint32_t wait_cycles, spi_instruction_address_trans_mode_t instruction_address_trans_mode)
{
    ...
    switch(instruction_length)
    {
        ...
        case 16:
            ...
            break;
        default:
            assert(!"Invalid instruction length");
            break;
    }
                                   ...
                                   const uint8_t *tx_buf, size_t tx_len, uint8_t *rx_buf, size_t rx_len)
{
    ...
    uint32_t *v_tx_buf = kmalloc(v_max_len * 4);
    uint32_t *v_rx_buf = kmalloc(v_max_len * 4);
    uint32_t i = 0;
    switch(frame_width)
    {
        case SPI_TRANS_INT:
                                ...
                                const uint32_t *cmd_buff, size_t cmd_len, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            buf = kmalloc(cmd_len * sizeof(uint32_t) + tx_len);
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len / 4; i++)
            ...
            break;
        case SPI_TRANS_SHORT:
            buf = kmalloc(cmd_len * sizeof(uint32_t) + tx_len / 2 * sizeof(uint32_t));
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len / 2; i++)
