...
/*void _gpiohs_set_drive_mode(uint8_t pin, gpio_drive_mode_t mode,int io_number) {
  ...
  volatile uint32_t *reg_d =
      !dir ? gpiohs->output_en.u32 : gpiohs->input_en.u32;
...
int gpiohs_pin_onchange_isr(void *userdata) {
  ...
  if (ctx->gpiohs_callback) ctx->gpiohs_callback(ctx->context);
...
void gpiohs_set_irq(uint8_t pin, uint32_t priority, void (*func)()) {
  ...
  plic_irq_register(IRQN_GPIOHS0_INTERRUPT + pin, gpiohs_pin_onchange_isr,
                    &(pin_instance[pin]));
